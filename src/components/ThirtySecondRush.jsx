...
const ThirtySecondRush = ({ windowSize }) => {
  ...
  useEffect(() => {
    if (firstLoad) {
      ...
      const nowPlus30 = now + 30 * 1000;
      setTimerStarted(now);
      setTimeToEnd(nowPlus30);
      setFirstLoad(false);
    }
  ...
  useEffect(() => {
    if (running) {
      if (!warningPlayed && timeToEnd - timerStarted < 5000) {
        if (soundsOn) playRandomSound('fiveSeconds');
      ...
      if (timeToEnd - timerStarted < 0) {
        ...
        showCorrectMatch();
        // setRunning(false);
        setFinalPauseStarted(true);
        if (!finalPauseStarted) {
          setTimeout(() => {
            setRunning(false);
            setClickable(true);

            const gameMode = `30SecondRush, ${pics ? 'pics' : ''}${words ? 'words' : ''}`;
            const previousBest = localStorage.getItem(gameMode) || 0;
            if (correct >= previousBest) {
              // New best score
              if (soundsOn) playRandomSound('newBest');
              localStorage.setItem(gameMode, correct);
  ...
  useEffect(() => {
    // console.log(setupNewQuestion, pics, words);
  ...
  const handleClick = (row, column) => {
    if (matchCorrect[row] === column) {
      ...
      // Subtract 3 seconds as a penalty
      setTimeToEnd(timeToEnd - 3 * 1000);
  ...
  const handleSkip = () => {
    ...
    // Subtract 1 second as a penalty
    setTimeToEnd(timeToEnd - 1 * 1000);
    setRedSquareOn(resetSquareOn);
    showCorrectMatch();
    setTimeout(() => {
      setSquareOn(resetSquareOn);
  ...
  const renderScoreboard = () => (
    <div style={styles.scoreboard}>
      ...
      <div style={styles.scorboardLine}>
        {`Time: ${formatTime(timeToEnd - timerStarted)}`}
      </div>
      <Button
...
export default ThirtySecondRush;
